			...
			for(int j = 0; j < game.getBlocks()[i].length; j++) {
				...
				switch(game.getBlocks()[i][j].getType()) {
					case Block.BLUE_PLAYER:
						//canvas.getGraphicsContext2D().drawImage(GraphicsUtil.getImg(), x, y, Settings.block,Settings.block);
					...
					case Block.POINT:
						canvas.getGraphicsContext2D().setFill(Color.YELLOW);
						canvas.getGraphicsContext2D().fillOval(x+Settings.block/4, y+Settings.block/4, Settings.block/2, Settings.block/2);
						break;
					case Block.WALL:
						canvas.getGraphicsContext2D().setFill(Color.BLUE);
